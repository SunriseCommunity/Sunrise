...
public class UserStatsService
{
    ...
    public async Task<long> GetUserCountryRank(int userId, GameMode mode)
    {
        ...
        var rank = await _redis.SortedSetRank(RedisKey.LeaderboardCountry(mode, (CountryCode)user.Country), userId);

        if (!rank.HasValue)
        {
            rank = await _redis.SortedSetRank(RedisKey.LeaderboardCountry(mode, (CountryCode)user.Country), userId);
            await SetUserRank(userId, mode);
        }
    ...
    private async Task<UserStats> SetUserRank(UserStats stats)
    {
        ...
        await _redis.SortedSetAdd(RedisKey.LeaderboardCountry(stats.GameMode, (CountryCode)user.Country),
            stats.UserId,
    ...
    private async Task RemoveUserRank(int userId, GameMode mode)
    {
        ...
        await _redis.SortedSetRemove(RedisKey.LeaderboardCountry(mode, (CountryCode)user.Country), userId);
    }
